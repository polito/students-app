...
import { TeachingStackParamList } from '@core/navigators/TeachingNavigator';
import {
  useBookExam,
  useCancelExamBooking,
  useGetExams,
} from '@core/queries/exams';
import { useGetPerson } from '@core/queries/people';
...
export const ExamScreen = () => {
  const navigation =
    useNavigation<NavigationProp<ExamsStackParamList, 'Exam'>>();
  const route = useRoute<RouteProp<ExamsStackParamList, 'Exam'>>();
  ...
  useEffect(() => {
    if (
      (routes[routes.length - 2]?.name as keyof TeachingStackParamList) ===
      'Course'
    ) {
