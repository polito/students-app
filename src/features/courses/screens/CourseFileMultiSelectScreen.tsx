...
export const CourseFileMultiSelectScreen = ({ route, navigation }: Props) => {
  const { courseId, mode, directoryId, initialSelectedIds } = route.params;
  ...
  const contextDownloadedIds = useMemo(() => {
    const ids = new Set<string>();
    for (const file of flatFileList) {
      ...
      const key = `${url}:${path}`;
      if (downloads[key]?.isDownloaded) {
        ids.add(file.id);
      }
    }
    return ids;
  }, [flatFileList, courseId, course?.name, getCourseFilePath, downloads]);

  const [dbFileIds, setDbFileIds] = useState<Set<string>>(new Set());

  const downloadedFileIdsFromDb = useMemo(() => {
    if (dbFileIds.size === 0) return contextDownloadedIds;
    if (contextDownloadedIds.size === 0) return dbFileIds;
    return new Set([...contextDownloadedIds, ...dbFileIds]);
  }, [contextDownloadedIds, dbFileIds]);
  ...
  useEffect(() => {
    ...
    fileDatabase
      ...
      .then(files => setDbFileIds(new Set(files.map(f => f.id))))
      .catch(() => setDbFileIds(new Set()));
  ...
  useEffect(() => {
    ...
    const validIds = initialSelectedIds.filter(id =>
      flatFileList.some(f => f.id === id),
    );
    if (validIds.length > 0) {
      setSelectedFileIds(prev => new Set([...prev, ...validIds]));
      hasAppliedInitialSelection.current = true;
    }
  }, [flatFileList, initialSelectedIds]);
  ...
  const handleDownloadPress = useCallback(() => {
    ...
    if (alreadyDownloadedCount > 0) {
      ...
      const isOneUpdate = alreadyDownloadedCount === 1;
      const isOneDownload = newCount === 1;
      const isZeroNew = newCount === 0;
      const messageKey = isZeroNew
        ? isOneUpdate
          ? 'courseFilesTab.updateDownloadedInfoMessageOnlyUpdate'
          : 'courseFilesTab.updateDownloadedInfoMessageOnlyUpdates'
        : isOneUpdate && isOneDownload
          ? 'courseFilesTab.updateDownloadedInfoMessage'
          : isOneUpdate
            ? 'courseFilesTab.updateDownloadedInfoMessageOneUpdate'
            : isOneDownload
              ? 'courseFilesTab.updateDownloadedInfoMessageOneDownload'
              : 'courseFilesTab.updateDownloadedInfoMessages';
  ...
  const handleRemovePress = useCallback(() => {
    ...
    InteractionManager.runAfterInteractions(showRemoveAlert);
  }, [
    flatFileList,
    selectedFileIds,
    downloadedFileIdsFromDb,
