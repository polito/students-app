...
type Props = NativeStackScreenProps<CoursesStackParamList, 'AssignmentUpload'>;

export const AssignmentUpload = ({ navigation, route }: Props) => {
  ...
  const handlePickedFile = (fileUri: string) => {
    navigation.navigate('AssignmentUploadConfirmation', {
      courseId,
      fileUri,
    });
  };
      ...
      <SectionList indented>
        ...
        <ListItem
          ...
          onPress={() => {
            takePicture()
              .then(image => {
                navigation.navigate('AssignmentPdfCreation', {
                  courseId,
